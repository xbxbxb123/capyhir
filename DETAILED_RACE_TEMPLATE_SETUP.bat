@ECHO OFF
TITLE Detailed Race Template by Omeruin, Credit to DrPvtSkittles and Sev
ECHO ==================================================
...
FOR /R . %%j in (*.patch, *.species, *.animation, *.config, *.structure, *.recipe, *.monstertype, *.monsterpart, *.object, *.frames, *.activeitem, *.npctype, *.statuseffect, *.legs, *.chest, *.head, *.cinematic, *.item, *.tenant) do (
	IF EXIST "%%j" (
		SET /P "=#" <nul
		SET textFile=%%j
		CALL:CONFIGURE
